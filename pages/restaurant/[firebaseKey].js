...
return (
    ...
    <a href={`mailto:${restaurantDetails.restaurantObject?.email}`}>{restaurantDetails.restaurantObject?.email}</a>
    <div>
    {restaurantDetails.entrees?.map((entree) => (
            <RestaurantCard key={entree.firebaseKey} entreeObj={entree} onUpdate={getTheRestaurantDetails} />
          ))}
        </div>
      </div>
    </div>
